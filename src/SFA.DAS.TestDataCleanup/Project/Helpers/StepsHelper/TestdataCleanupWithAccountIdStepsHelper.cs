...
public class TestdataCleanupWithAccountIdStepsHelper
{
    ...
    {
        return AddDbName(new TestDataCleanupComtSqlDataHelper(_objectContext, _dbConfig).CleanUpComtTestData(_greaterThan, _lessThan, _easaccountidsnottodelete), "comt");
    }

    internal (List<string> usersdeleted, List<string> userswithconstraints) CleanUpPrelTestData()
    {
        return AddDbName(new TestDataCleanUpPrelDbSqlDataHelper(_objectContext, _dbConfig).CleanUpPrelTestData(_greaterThan, _lessThan, _easaccountidsnottodelete), "prel");
    }

    internal (List<string> usersdeleted, List<string> userswithconstraints) CleanUpPfbeTestData()
    {
        return AddDbName(new TestDataCleanUpPfbeDbSqlDataHelper(_objectContext, _dbConfig).CleanUpPfbeTestData(_greaterThan, _lessThan, _easaccountidsnottodelete), "pfbe");
    }

    internal (List<string> usersdeleted, List<string> userswithconstraints) CleanUpEmpFcastTestData()
    {
        return AddDbName(new TestDataCleanUpEmpFcastSqlDataHelper(_objectContext, _dbConfig).CleanUpEmpFcastTestData(_greaterThan, _lessThan, _easaccountidsnottodelete), "fcast");
    }

    internal (List<string> usersdeleted, List<string> userswithconstraints) CleanUpEmpFinTestData()
    {
        return AddDbName(new TestDataCleanUpEmpFinSqlDataHelper(_objectContext, _dbConfig).CleanUpEmpFinTestData(_greaterThan, _lessThan, _easaccountidsnottodelete), "empfin");
    }

    internal (List<string> usersdeleted, List<string> userswithconstraints) CleanUpRsvrTestData()
    {
        return AddDbName(new TestDataCleanUpRsvrSqlDataHelper(_objectContext, _dbConfig).CleanUpRsvrTestData(_greaterThan, _lessThan, _easaccountidsnottodelete), "rsvr");
    }

    internal (List<string> usersdeleted, List<string> userswithconstraints) CleanUpEmpIncTestData()
    {
        return AddDbName(new TestDataCleanUpEmpIncSqlDataHelper(_objectContext, _dbConfig).CleanUpEmpIncTestData(_greaterThan, _lessThan, _easaccountidsnottodelete), "empinc");
    }

    internal (List<string> usersdeleted, List<string> userswithconstraints) CleanUpEasLtmTestData()
    {
        return AddDbName(new TestDataCleanUpEasLtmcSqlDataHelper(_objectContext, _dbConfig).CleanUpEasLtmTestData(_greaterThan, _lessThan, _easaccountidsnottodelete), "easltm");
    }
