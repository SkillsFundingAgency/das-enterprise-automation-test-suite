...
namespace SFA.DAS.TestDataCleanup.Project.Helpers
{
    public class TestDataCleanUpPfbeDbSqlDataHelper : ProjectSqlDbHelper
    {
        ...
        {
            return await CleanUpTestData(() => GetPfbeAccountids(greaterThan, lessThan, easaccountids), (x) => CleanUpPfbeTestData(x));
        }

        internal async Task CleanUpPfbeTestData(List<string> accountIdToDelete)
        {
            await CleanUpTestData(accountIdToDelete, (x) => $"Insert into #accountids values ({x})", "create table #accountids (id int)", "EasPfbeTestDataCleanUp");
        }
