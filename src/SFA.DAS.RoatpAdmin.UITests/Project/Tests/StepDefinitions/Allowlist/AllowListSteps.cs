...
namespace SFA.DAS.RoatpAdmin.UITests.Project.Tests.StepDefinitions.Allowlist
{
    ...
    public class AllowListSteps
    {
        ...
        public void ThenTheAdminIsAbleToAddAUkprnToAllowList()
        {
            _allowListPage = _allowListPage.AddUkprnToAllowList();
        }

        [Then(@"the admin is not able to add a ukprn exists in allow list")]
        public void ThenTheAdminIsNotAbleToAddAUkprnExistsInAllowList()
        {
            _allowListPage = _allowListPage.VerifyDuplicateUkrpnNotAdded();
        }

        [Then(@"the admin is able to remove a ukprn from the allow list")]
        public void ThenTheAdminIsAbleToRemoveAUkprnFromTheAllowList()
        {
            _confirmationPage = _allowListPage.RemoveUkprnFromAllowlist().SelectYesRemoveUkprn();
        }
    }
}
