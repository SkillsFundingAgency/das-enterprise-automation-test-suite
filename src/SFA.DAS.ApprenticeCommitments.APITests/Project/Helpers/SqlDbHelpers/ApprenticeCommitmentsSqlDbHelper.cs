...
namespace SFA.DAS.ApprenticeCommitments.APITests.Project.Helpers.SqlDbHelpers
{
    public class ApprenticeCommitmentsSqlDbHelper : SqlDbHelper
    {
        ...
        public void DeleteRevisionAndApprenticeshipTableData(string apprenticeId) => ExecuteSqlCommand(
            $"DELETE FROM Revision WHERE ApprenticeshipId in (SELECT ApprenticeshipId from Registration WHERE ApprenticeId = '{apprenticeId}')" +
            $"DELETE FROM Apprenticeship WHERE ApprenticeId = '{apprenticeId}'");

        public void DeleteRegistrationTableData(string apprenticeId) => ExecuteSqlCommand($"DELETE FROM Registration WHERE ApprenticeId = '{apprenticeId}'");
