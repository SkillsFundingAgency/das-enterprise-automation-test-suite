...
Scenario: Resume 1 - Learner Stopped COC triggered and then Resumed in later period 
    ...
    When Learner data is updated with Price Episode End Date which is before the due date of the paid earning in Period R08 2021
    And the Learner Match is run in Period R08 2021
    And the Unpaid Earnings are Archived
    And the paid earnings of £1000 is marked as required a clawback in the currently active collection period
    And Learner data is updated with Price Episode Start Date which is on or after Previous Price Episode start date AND on or before the Previous Price Episode end date
    And the Learner Match is run in Period R08 2021
...
Scenario: Resume 3 - Learner Stopped COC triggered one day after the due date of the First Earning and then Resumed in later period
    Given an existing apprenticeship incentive with learning starting on 01-Nov-2020 and ending on 31-Jul-2021
    And a payment of £1000 sent in Period R06 2021
