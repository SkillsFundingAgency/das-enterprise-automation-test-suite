...
,case when  (os.EffectiveTo IS NULL OR os.EffectiveTo > GETDATE() AND os.status = 'Live')  THEN 1 ELSE 0 END standard_Active
                        ...
                        JOIN [dbo].[Contacts] c on o.EndPointAssessorOrganisationId = c.EndPointAssessorOrganisationId and c.email = @endPointAssessorEmail
...
SELECT TOP 1  
il1.uln, il1.stdcode, GivenNames, familyname FROM StandardsList
JOIN  il1  on  il1.stdcode = StandardsList.stdcode
