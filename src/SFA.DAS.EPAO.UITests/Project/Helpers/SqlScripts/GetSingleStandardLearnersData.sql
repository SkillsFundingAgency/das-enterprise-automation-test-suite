...
AS
(
select count (StdCode) OVER (PARTITION BY uln, GivenNames, FamilyName) multi, uln, stdcode, familyname, GivenNames from dbo.Learner
)
SELECT TOP 1
...
AND (CASE WHEN learner.VersionConfirmed = 1 THEN 1 ELSE 0 END) = __VersionConfirmed__   -- Version is/isn't confirmed
AND (CASE WHEN ISNULL(learner.CourseOption,'') = '' THEN 0 ELSE 1 END ) = __OptionSet__  -- option is/isn't set
order by Newid() desc
