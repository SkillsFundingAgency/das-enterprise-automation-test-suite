...
using RegHelper = SFA.DAS.FrameworkHelpers.RegexHelper;
...
namespace SFA.DAS.UI.FrameworkHelpers
{
    public class RegexHelper
    {
        ...
        public static string Replace(string value, List<string> pattern) => RegHelper.TrimAnySpace(Regex.Replace(value, $@"{pattern.ToString("|")}", string.Empty));

        public static int GetMaxNoOfPages(string question)
        {
            var match = Regex.Match(question, @"of [0-9]*", RegexOptions.None);

            return int.Parse(RegHelper.TrimAnySpace(Regex.Replace(match.Value, @"of", string.Empty)));
        }
        ...
        public static string GetPublicAccountId(string text)
        {
            ...
            return match.Success ? RegHelper.TrimAnySpace(Regex.Replace(match.Value, @"Account ID:", string.Empty)) : text;
        }
        ...
        public static string GetApplicationReference(string value)
        {
            ...
            return match.Success ? RegHelper.TrimAnySpace(match.Value) : value;
        }
        ...
        public static string GetVacancyCurrentWage(string value)
        {
            ...
            return match.Success ? RegHelper.TrimAnySpace(Regex.Replace(match.Value, @"£", string.Empty)) : value;
        }
