...
public class RoatpV2SqlDataHelper(ObjectContext objectContext, DbConfig dbConfig) : SqlDbHelper(objectContext, dbConfig.ManagingStandardsDbConnectionString)
{
    ...
    internal List<string> GetCourseTitlesthatProviderDeosNotOffer(string ukprn) => GetCourses($"SELECT Title FROM [dbo].[Standard] WHERE LarsCode NOT IN ({ProviderCourseQuery(ukprn)}) order by NEWID();");
